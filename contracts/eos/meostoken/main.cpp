...
[[eosio::action]] void meostoken::issue(issue_args payload) // name to, asset quantity, string memo
{
...
[[eosio::action]] void meostoken::vtransfer(vtransfer_args payload) // name from, name to, asset quantity, string memo
{
...
[[eosio::action]] void meostoken::transfer(name from, name to, asset quantity, string memo) {
    require_auth(from);
...
[[eosio::action]] void meostoken::withdraw(withdraw_args payload) // name to, asset quantity
{
    require_vaccount(payload.from);
    // require_auth(to);
...
void meostoken::add_balance(name owner, asset value, name ram_payer)
{
...
void meostoken::sub_balance(name owner, asset value)
{
...
void meostoken::add_cold_balance(name owner, asset value, name ram_payer)
{
...
void meostoken::sub_cold_balance(name owner, asset value)
{
...
bool meostoken::timer_callback(name timer, std::vector<char> payload, uint32_t seconds)
{
